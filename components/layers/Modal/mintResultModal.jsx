...
export default function mintResultModal({ isOpen, onClose, setData, setShowResult, data, transactionHash }) {
    ...
    return (
        <Modal isOpen={isOpen} onClose={() => { onClose(); setData(""); setShowResult(false) }} className='transparent-div'>
            ...
            <ModalContent rounded={"xl"} backgroundColor={"transparent"}>
                        ...
                        <ModalBody pt={4} rounded={"xl"} backgroundColor={"#0E1114"} pb={4} position={"relative"} color={"white"}>
                            ...
                            <Grid templateColumns='repeat(2, 1fr)'>
                                <GridItem colSpan={1}>
                                    ...
                                    <Text mt={1}>{country[data]?.name || ""}</Text>
                                ...
                                <GridItem colSpan={1}>
                                    ...
                                    <Text mt={1} >{country[data]?.attributes[2].value || ""}%</Text>
                                    <Box mt={4} pt={4} borderTop={"1px solid rgba(41,49,61,1)"}>≈$68,000</Box>
                                </GridItem>
                            </Grid>
                        </ModalBody>
                    </Box>
                ...
                <Button
                    ...
                    <Text fontWeight={700} fontSize={18} color={"white"} >Collect</Text>
                </Button>
                ...
                <Box className="rounded-xl mt-4 mx-auto bg-gradient-to-r p-[2px] from-[#80E8DD] via-[#7CC2F6] to-[#D855A6]" mt={5} w={"full"}>
                    <Box className="flex flex-col justify-between h-full rounded-xl p-1" bg='#0E1114'>
                        <Button onClick={() => { window.open(`https://testnet.bscscan.com/tx/${transactionHash}`, '_blank') }} variant='unstyle' color={"white"} p={5} fontWeight={700} fontSize={18} >View on BSCscan</Button>
                    </Box>
                </Box>
            </ModalContent>
        </Modal>
