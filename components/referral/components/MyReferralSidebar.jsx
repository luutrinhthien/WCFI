...
const MyReferralSidebar = ({ address }) => {
  ...
  return (
    ...
    <Box bg="#0E1114" p={5} rounded="lg" color="rgb(99 117 146)">
      ...
      <Box mt={5} display="flex" flexDirection={{ base: "column", lg: "row" }} gap={5}>
        <Box flex="1">
          <Text fontSize="lg" fontWeight="bold">Total Earned</Text>
          <Text mt={3} color="white" fontWeight="bold" display="flex" alignItems="center" gap={2}>
            <Image src="https://d1fdloi71mui9q.cloudfront.net/iMWarJIZQl6F69HPBmm3_0G9qdRKlYzBNic51" alt="" w={6} h={6} />
            {(data && parseFloat(data.commission?.toString()).toFixed(2)) || 0}
          </Text>
        </Box>

        <Box bg="#191D24" rounded="lg" p={3} flex="1" display="flex" flexDirection="column" alignItems="center" justifyContent="center">
          <Text>Available to Withdraw on</Text>
          <Text mt={1} fontWeight="bold">25/12/2022</Text>
        </Box>
      </Box>

      {enableWithdraw ? (
        <Button bg="#191D24" mt={5} py={2} fontSize="lg" w="full">Withdraw</Button>
      ) : (
